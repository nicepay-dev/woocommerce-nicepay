...
include_once ('NicepayConfigVAV2.php');

use libraryVAV2\Config as VAV2Config;

class NicepayRequestorVAV2 {
    public $sock = 0;
    public $apiUrl;
    public $port = 443;
    public $status;
    ...
    public function operation($type) {
        if ($type == "requestVAV2") {
            $this->apiUrl = VAV2Config::NICEPAY_GENERATE_VA_URL;
            // } else if ($type == "creditCard") {
            // $this->apiUrl = VAV2Config::NICEPAY_REQ_CC_URL;
        } else if ($type == "checkPaymentStatus") {
            $this->apiUrl = VAV2Config::NICEPAY_ORDER_STATUS_URL;
        } // else if ($type == "cancelVA") {
        // $this->apiUrl = NVAV2Config::ICEPAY_CANCEL_VA_URL;
        // }
    }
    ...
    public function openSocket() {
        ...
        $tryCount = 0;
        if (! $this->sock = @fsockopen ("ssl://".$host, $this->port, $errno, $errstr, VAV2Config::NICEPAY_TIMEOUT_CONNECT)) {
            while ($tryCount < 5) {
                if ($this->sock = @fsockopen("ssl://".$host, $this->port, $errno, $errstr, VAV2Config::NICEPAY_TIMEOUT_CONNECT)) {
                    return true;
                }
    ...
    public function apiRequest($Data) {
        ...
        if($this->sock) {
            ...
            while ( ! feof ($this->sock ) && ! $timeout) {
                ...
                if ($diff >= VAV2Config::NICEPAY_TIMEOUT_READ) {
                    $timeout = true;
                }
                if ($IsHeader) {
            ...
            if ($timeout) {
                $this->errorcode = VAV2Config::NICEPAY_READ_TIMEOUT_ERR;
