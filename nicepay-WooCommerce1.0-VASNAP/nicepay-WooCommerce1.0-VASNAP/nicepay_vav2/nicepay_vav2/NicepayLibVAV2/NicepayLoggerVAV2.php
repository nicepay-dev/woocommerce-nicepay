...
class NicepayLoggerVAV2 {
    ...
    function StartLog($dir, $mid) {
        ...
        $logfile = $dir. "/logs/".VAV2Config::NICEPAY_PROGRAM."_".$this->type."_".$mid."_".date("ymd").".log";
        $this->handle = fopen( $logfile, "a+" );
        if(!$this->handle) {
            return false;
        }

        $this->WriteLog("START ".VAV2Config::NICEPAY_PROGRAM." ".$this->type." (V".VAV2Config::NICEPAY_VERSION."B".VAV2Config::NICEPAY_BUILDDATE."(OS:".php_uname('s').php_uname('r').",PHP:".phpversion()."))");
        return true;
    }
